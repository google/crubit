...
/// Translation-unit-scoped side table recording the flow-narrowed nullability
/// of by-value lambda captures, keyed by (closure record, capture field). Only
/// captures proven NonNull or Nullable at the capture site are recorded; the
/// recorded kind is used to model the capture inside the lambda body. Absence
...
///
/// If `CaptureMap` is non-null, it is populated when analyzing a function that
/// constructs lambdas and consulted when analyzing a lambda call operator, to
/// preserve the nullability of by-value captures.
///
...
diagnosePointerNullability(
    ...
    LambdaCaptureNullabilityMap* absl_nullable CaptureMap = nullptr);
