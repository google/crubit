...
// member nullability) may no longer hold. A const `&&`-qualified method cannot
// move from members, so it is excluded.
...
// Returns true if an object of this class may have its nonnull pointer fields
// left in a null (moved-from) state before it is destroyed -- either via a move
// operation (a move constructor or move assignment operator, including
// implicitly-declared ones) or via a non-const `&&`-qualified method. A move
// operation may null out smart-pointer members; a non-const `&&`-qualified
// method may null out both smart- and raw-pointer members.
static bool classMayBeMovedFrom(const CXXRecordDecl& RD) {
  ...
  for (const CXXMethodDecl* Method : RD.methods()) {
    ...
    if (const auto* Ctor = dyn_cast<CXXConstructorDecl>(Method);
        Ctor != nullptr && Ctor->isMoveConstructor())
      return true;
    if (Method->isMoveAssignmentOperator() ||
        isNonConstRValueRefQualified(*Method))
      return true;
  }
  return false;
}
...
static void diagnoseNonnullPointerFieldNullableAtExit(
  ...
  // Running a non-const &&-qualified method requires moving from the object,
  // after which its invariants (including member nullability) may no longer be
  // valid. A const &&-qualified method cannot move from members, so its exit
...
// operation or a non-const `&&`-qualified method may have nulled them out
// before destruction. Returns empty if the class cannot be moved from.
