...
// Records the flow-sensitive nullability of each by-value pointer capture of
// the lambda constructed at this program point, keyed by (closure record,
// capture field).
...
diagnosePointerNullability(const ValueDecl* absl_nonnull VD,
  ...
  // If analyzing a lambda call operator, install the capture-time
