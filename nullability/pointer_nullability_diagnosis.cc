...
static SmallVector<PointerNullabilityDiagnostic> diagnoseNonnullExpected(
  ...
  if (isCapturedVariableOrMemberAccess(E))
    return {
        {.Code = PointerNullabilityDiagnostic::ErrorCode::ExpectedNonnull,
         ...
         .NoteRange = Range,
         .NoteMessage =
             "This pointer is captured and dereferenced in a lambda. For a "
             "by-value capture, null-check it before the capture or inside the "
             "lambda body. For a by-reference or *this capture, capture the "
             "pointee instead."}};
  ...
  if (const FieldDecl* Field =
          fieldTreatedAsNullableAtDestructorEntry(*E, State.Lattice)) {
...
static bool classMayBeMovedFrom(const CXXRecordDecl& RD) {
  ...
  for (const CXXMethodDecl* Method : RD.methods()) {
    ...
    for (const ParmVarDecl* Param : Method->parameters()) {
      if (const auto* RRT = Param->getType()->getAs<RValueReferenceType>()) {
        // A `const RD&&` parameter cannot be moved from.
        if (RRT->getPointeeType().isConstQualified()) continue;
        if (const CXXRecordDecl* ParamRD =
...
computeFieldsToTreatAsNullableAtDestructorEntry(
  ...
  for (const FieldDecl* Field : RD->fields()) {
    ...
    // A const field is copied (not moved) by a defaulted move operation and
    // cannot be nulled by a &&-qualified method, so it is never moved-from and
    // remains nonnull at destructor entry.
...
// field). Only captures provably NonNull or Nullable at the capture site are
// recorded; Unknown captures fall back to their declared type.
static void recordLambdaCaptureNullability(
  ...
  for (; Capture != LE->capture_end() && Field != Closure->field_end() &&
       ...
       ++Capture, ++Field, ++Init) {
    // Only by-value captures of variables are frozen at construction time.
    // By-reference and this/*this captures alias mutable outer state that the
    // enclosing body may reset to null later, so they must stay nullable.
    if (Capture->getCaptureKind() != LCK_ByCopy) continue;
    if (!Capture->capturesVariable()) continue;
    const Expr* InitExpr = *Init;
    if (InitExpr == nullptr) continue;
    // Only pointer-typed captures (raw or smart) carry nullability; capturing a
    // whole struct/class by value has no pointer null state to preserve.
    if (!isSupportedPointerType(InitExpr->getType())) continue;
...
computeCapturedVarNullability(const CXXRecordDecl& Closure,
  ...
  for (; Capture != Closure.captures_end() && Field != Closure.field_end();
       ++Capture, ++Field) {
    if (!Capture->capturesVariable()) continue;
    auto It = CaptureMap.find({&Closure, *Field});
