...
/// behaviors in a format-independent representation of the
/// behaviors: PointerNullState, Slots, Formulas, etc. instead of CFG/AST or
/// Proto (but originally derived from the CFG/AST).
class CollectorFromNullabilityBehavior {
 public:
  virtual ~CollectorFromNullabilityBehavior() = default;

  /// Collects information for Nonnull-ness of one InferableSlot out of several
  /// candidates, derived from the necessity that `MustBeTrue` must be true.
  ///
  /// Used when we have reason to believe that `MustBeTrue` can be made true by
  /// marking some candidate slot Nonnull.
  virtual void mustBeTrueByMarkingNonnull(const Formula& MustBeTrue,
                                          const SerializedSrcLoc& Loc,
                                          Evidence::Kind EvidenceKind) = 0;

  /// Collects information for Nullable-ness for potentially multiple
  /// candidate slots, derived from the necessity that `MustBeTrue` must be
  /// true.
  ///
  ...
  /// declaration (e.g. field, variable, parameter), collects information
  /// based on constraints of the type of the assignment's left-hand side (LHS)
  /// and any inferable-slots which may influence the value's nullability. For
  /// example, if the assignee is Nonnull, collects the constraint that the RHS
  /// is nonnull as well.
  ///
  ...
  ///   nullability vector.
  ///
  /// * `LHSFingerprint` is the fingerprint of the "LHS" ValueDecl whose
  ...
  /// Collects information from assignments, specifically about the nullability
  /// of an assignee based on the nullability of the RHS value.
  ///
  ...
  /// Example:
  /// ```
  /// void target(int* p, int* q, int* _Null_unspecified r) {
  ///   p = nullptr;
  ///   if (!r) {
  ///     q = r;
  ///   }
  ...
  /// }
  /// ```
  /// From the above, we collect information from each of the assignments of `p`
  /// and `q` that they were ASSIGNED_FROM_NULLABLE and information from the
  ...
  /// `TargetSlot`, collect information to indicate that the nullability slot
  /// must match the type nullability `NullabilityToMatch`.
  /// For example, `TargetUSR` and `TargetSlot` may be for a parameter of
  /// a function, while `NullabilityToMatch` is the nullability of an argument.
  ///
  /// * `CrossesFromTestToNontest` indicates that the information comes from
  ///   a reference from test code to non-test code.
  ///
  /// * `CollectionLoc` is the beginning source location of the expression
  ///    from which `NullabilityToMatch` was derived.
  ...
  /// Collect information to indicate that there is behavior where
  /// `TargetNullability` must match `NullabilityToMatch`.
  /// For example, TargetNullability might be the nullability of a function
  /// call argument, while `NullabilityToMatch` is the nullability of the
  /// corresponding parameter.
  ///
  /// * `FingerprintOfNullabilityToMatch` is the fingerprint of the ValueDecl
  ///   from which `NullabilityToMatch` was derived. This is useful when the
  ///   ValueDecl's nullability annotation may be overridden in future rounds of
  ///   inference. When the origin of `NullabilityToMatch` does not change in
  ///   future rounds of inference, this can be nullopt.
  ///
  /// * `CollectionLoc` is the beginning source location of the expression
  ///    from which `TargetNullability` was derived.
  virtual void collectMustHaveInvariantTypeNullability(
  ...
  /// Collects information for parameter nullability based on arguments passed
  /// at call sites. Considers two distinct cases, based on the setting of
  ...
  /// Collects information from an operation that requires two pointer operands
  /// to differ in value. Specifically, considers cases where one operand is
  /// known statically to be null, in which case we have evidence that the other
  /// is Nonnull. The canonical example is `CHECK_NE(p, q)`, but other
  /// operations could conceivably provide the same potential evidence.
  ///
  ...
  /// Collects information constrainting return-type Nullability for a
  /// function (identified by `FunctionUSR`), from e.g., a `return x;`
  /// statement at the given `ReturnLoc` where `x` has the given
  /// `ReturnNullState`.
...
// Collects and emits evidence from extracted nullability behaviors.
