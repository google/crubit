...
static void appendBaseUSRsIfNotCrossingTestBoundary(
  ...
  switch (FlowDirection) {
    ...
    case VirtualMethodEvidenceFlowDirection::kFromDerivedToBase:
      if (auto It = Index.Bases.find(USR); It != Index.Bases.end()) {
        appendBaseUSRsIfNotCrossingTestBoundary(USR, Index.IsDefinedInTest,
                                                It->second, Results);
      }
      return Results;
...
static std::vector<InferableSlot> gatherInferableSlots(
  ...
  if (NotTestMainFileLocFilter != nullptr) {
    bool IsTargetInTestMainFile = !NotTestMainFileLocFilter->check(
        TargetAsFunc != nullptr ? TargetAsFunc->getBeginLoc()
                                : TargetStmt.getBeginLoc());
