...
const FieldDecl* absl_nullable fieldTreatedAsNullableAtDestructorEntry(
  ...
  const Expr* Exp = E.IgnoreParenImpCasts();
  ...
  if (const auto* Op = dyn_cast<CXXOperatorCallExpr>(Exp);
      Op != nullptr && Op->getOperator() == OO_Arrow && Op->getNumArgs() == 1)
    Exp = Op->getArg(0)->IgnoreParenImpCasts();
...
std::optional<NullabilityKind> getCapturedVarNullability(
  ...
  if (DRE == nullptr || !DRE->refersToEnclosingVariableOrCapture())
    return std::nullopt;
  if (auto It = Vars.find(DRE->getDecl()); It != Vars.end()) return It->second;
  return std::nullopt;
}
