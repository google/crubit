...
#include <memory>
    ...
    class target {
     public:
      target(target&&) = default;
      ~target() {
        // `some_resource_->x` lowers to `some_resource_.operator->()->x`. The
        // field may have been moved from before destruction, so this
...
#include <memory>
    ...
    class target {
     public:
      // A non-const `&&`-qualified method makes the class movable-from.
      void Finalize() && {}
      ~target() {
