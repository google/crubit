...
TEST void topPointerNarrowingViaTernary(int* _Nullable P, int N, bool B) {
  int X = 17;
  for (int I = 0; I < N; ++I) {
    if (B) P = &X;
  }
  bool V = true;
  bool IsNull = V ? P == nullptr : false;
  if (IsNull) __builtin_abort();
  nonnull(P);
}

TEST void topPointerNarrowingUnconstrainedV(int* _Nullable P, int N, bool B,
                                            bool V) {
  int X = 17;
  for (int I = 0; I < N; ++I) {
    if (B) P = &X;
...
TEST void topPointerNarrowingMultiplePointers(int* _Nullable P,
                                              int* _Nullable Q, int N, bool B) {
  int X = 17;
  for (int I = 0; I < N; ++I) {
    if (B) P = &X;
    if (B) Q = &X;
  ...
  bool V = true;
  if (V && (P == nullptr || Q == nullptr)) __builtin_abort();
  nonnull(P);
  nonnull(Q);
}
