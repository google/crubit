...
void initPointerFromTypeNullability(
  ...
  // nullable at destructor entry (because a move operation or non-const
  // `&&`-qualified method may have nulled it out before destruction), model its
  // null state as nullable instead of using its declared (nonnull) type. This
  // affects only the first derivation of the field's null state; subsequent
  // assignments and null checks in the destructor body are handled normally by
  // the flow analysis.
