...
def cc_and_rs_deps_for_bindings(is_toolchain):
    """Returns the attributes for the _cc_deps_for_bindings and _rs_deps_for_bindings attributes.

    Args:
      is_toolchain: Whether the bindings are for building the Rust toolchain.
        Specifically, this means _not_ using dyn_future, which is not necessary and would create a
        dep cycle.

    Returns:
      A dictionary containing entries `_cc_deps_for_bindings` and
      `_rs_deps_for_bindings` mapping to  `attr.label_list` with appropriate default
      values.
    """
...
cc_bindings_from_rust_aspect = aspect(
    ...
    } | cc_and_rs_deps_for_bindings(is_toolchain = False),
    toolchains = [
...
cpp_api_from_rust_toolchain_bindings = rule(
    ...
    } | cc_and_rs_deps_for_bindings(is_toolchain = True),
    toolchains = [
        "@rules_rust//rust:toolchain_type",
        config_common.toolchain_type("//cc_bindings_from_rs/bazel_support:toolchain_type", mandatory = False),
    ] + use_cpp_toolchain(),
