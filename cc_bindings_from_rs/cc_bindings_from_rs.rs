...
/// This function exists so we can guard it behind a feature flag and should be cleand up once we've migrated to rmetas entirely.
fn run_with_rmetas(cmdline: &Cmdline) -> Result<()> {
    ...
    let Some(matches) = rustc_driver::handle_options(&early_dcx, &at_args) else {
        panic!("TODO: Real error handling on invalid options {at_args:?}")
    };

    // We do not support unstable options at the moment.
    // We just need an instsance of this struct so we can reuse logic in rustc rather than duplicate
    // it here.
    ...
    let Some(crate_name) = cmdline.source_crate_name.as_ref() else {
        early_dcx.early_fatal("--source-crate-name must be provided when using rmetas")
    };
