...
pub struct Cmdline {
    ...
    /// This flag is deprecated, use `crubit_support_path_format` instead.
    #[clap(long, value_parser, value_name = "STRING", default_value_t = String::from(""))]
    ...
    // TODO(b/295304963): Remove once Crosstool unstable has CL/566667051.
    ...
    default_value_t = String::from("\"support/public/{header}\""))]
    // TODO(b/295304963): Remove the default value once Crosstool unstable has CL/566667051.
...
Usage: cc_bindings_from_rs_unittest_executable [OPTIONS] --h-out <FILE> --rs-out <FILE> --clang-format-exe-path <FILE> --rustfmt-exe-path <FILE> [-- <RUSTC_ARGS>...]
...
Options:
      ...
      --crubit-support-path <STRING>
          Path to the `crubit/support` directory in a format that should be used in the `#include` directives inside the generated C++ files. Example: "crubit/support". This flag is deprecated, use `crubit_support_path_format` instead [default: ]
      --crubit-support-path-format <STRING>
          This is the format to `#include` Crubit C++ support library headers, using `{header}` as the  placeholder. Example: `<crubit/support/{header}>` will produce `#include <crubit/support/hdr.h>` [default: "support/public/{header}"]
