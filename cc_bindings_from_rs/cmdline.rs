...
Options:
      ...
      --default-features <CRUBIT_FEATURES>
          The default feature flags enabled for all crates
      ...
      --crate-disabled-feature <CRATE_NAME=CRUBIT_FEATURE>
          Feature flags disabled for a given crate. Keys are crate names, and values are feature flags. All crates must have their features fully specified, and consistently across rustc invocations, or the behavior is undefined. As a special case, the crate name `self` refers to the current crate, whose bindings are being generated.
          
          If a feature is specified in either `default-features` or `crate-feature`, and also in `crate-disabled-feature`, the latter takes precedence.
