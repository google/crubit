...
pub fn take_result_drop_no_default_tuple(val: (Result<(), DropNoDefault>,)) -> bool {
    val.0.is_ok()
}

pub fn return_result_drop_no_default_tuple(success: bool) -> (Result<u8, DropNoDefault>,) {
    if success {
        (Ok(42),)
    } else {
        (Err(DropNoDefault(100)),)
    }
}
