...
struct alignas(4) CRUBIT_INTERNAL_RUST_TYPE(
 ...
 private:
  ...
  ::std::int64_t* some_ptr() noexcept {
    return reinterpret_cast<::std::int64_t*>(storage_ + 4);
  }
  ::std::int64_t const* some_const_ptr() const noexcept {
    return reinterpret_cast<::std::int64_t const*>(storage_ + 4);
  }
  void set_some_tag() noexcept { set_tag(1); }
  constexpr void set_none_tag() noexcept { set_tag(kNoneVal); }
  constexpr bool is_none() const noexcept { return tag() == kNoneVal; }
...
struct alignas(4) CRUBIT_INTERNAL_RUST_TYPE(
 ...
 private:
  ...
  ::rs_std::isize* some_ptr() noexcept {
    return reinterpret_cast<::rs_std::isize*>(storage_ + 4);
  }
  ::rs_std::isize const* some_const_ptr() const noexcept {
    return reinterpret_cast<::rs_std::isize const*>(storage_ + 4);
  }
  void set_some_tag() noexcept { set_tag(1); }
  constexpr void set_none_tag() noexcept { set_tag(kNoneVal); }
  constexpr bool is_none() const noexcept { return tag() == kNoneVal; }
...
struct CRUBIT_INTERNAL_RUST_TYPE(
    ":: option_golden :: OverlappingOptions") alignas(4) [[clang::trivial_abi]]
...
struct alignas(4) CRUBIT_INTERNAL_RUST_TYPE(
 ...
 private:
  ...
  ::rs_std::usize* some_ptr() noexcept {
    return reinterpret_cast<::rs_std::usize*>(storage_ + 4);
  }
  ::rs_std::usize const* some_const_ptr() const noexcept {
    return reinterpret_cast<::rs_std::usize const*>(storage_ + 4);
  }
  void set_some_tag() noexcept { set_tag(1); }
  constexpr void set_none_tag() noexcept { set_tag(kNoneVal); }
  constexpr bool is_none() const noexcept { return tag() == kNoneVal; }
...
static_assert(
    sizeof(OverlappingOptions) == 28,
    "Verify that ADT layout didn't change since this header got generated");
static_assert(
    alignof(OverlappingOptions) == 4,
    "Verify that ADT layout didn't change since this header got generated");
...
inline void OverlappingOptions::__crubit_field_offset_assertions() {
  ...
  static_assert(8 == offsetof(OverlappingOptions, sixty_four));
  static_assert(20 == offsetof(OverlappingOptions, thirty_two));
}
static_assert(
    sizeof(ZStream) == 4,
    "Verify that ADT layout didn't change since this header got generated");
...
inline constexpr ::std::uint32_t rs_std::Option<::std::int64_t>::tag()
    const& noexcept {
  ::std::array<unsigned char, sizeof(::std::uint32_t)> __bytes = {};
  for (::std::size_t i = 0; i < sizeof(::std::uint32_t); ++i) {
    __bytes[i] = storage_[0 + i];
