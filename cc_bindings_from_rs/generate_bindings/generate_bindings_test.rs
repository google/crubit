...
#[test]
fn test_naming_conflict_suppression_strict() {
    let test_src = r#"
        pub fn foo() {}
    ...
    test_generated_bindings(test_src, |bindings| {
        ...
        // Both `foo` function and `foo` module should be suppressed, each with a
        // conflict comment emitted in the parent (`rust_out`) namespace.
        assert_cc_matches!(
            bindings.cc_api,
            quote! {
                ...
                namespace rust_out {
                    ...
                    __COMMENT__ "Error generating bindings for module `foo` defined at <crubit_unittests.rs>;l=4: Naming conflict in C++: item conflicts with another item having the same C++ name"
                    ...
                    __COMMENT__ "Error generating bindings for function `foo` defined at <crubit_unittests.rs>;l=2: Naming conflict in C++: item conflicts with another item having the same C++ name"
                    ...
                }
            }
        );

        // `bar` is silently suppressed because its ancestor `foo` module conflicts,
        // and the `rust_out::foo` namespace is never opened.
        let cc_api_str = bindings.cc_api.to_string();
        assert!(!cc_api_str.contains("namespace rust_out :: foo"));
        assert!(!cc_api_str.contains("void bar("));
    });
}

#[test]
fn test_naming_conflict_resolution_via_cpp_name() {
    let test_src = r#"
        #[doc="CRUBIT_ANNOTATE: cpp_name=foo_func"]
        pub fn foo() {}
    ...
    test_generated_bindings(test_src, |bindings| {
        ...
        // Neither should be suppressed (except module which returns Ok(None) but generates namespace).
        // `fn foo` should be generated as `foo_func`.
        // `bar` should be generated in `namespace foo`.
