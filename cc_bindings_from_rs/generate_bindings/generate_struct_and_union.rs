...
use crate::generate_function_thunk::{generate_thunk_decl, generate_thunk_impl};
use crate::{
...
pub fn into_trait_impls_by_destination<'tcx>(
    ...
    for into_impl_id in impls_iter {
        ...
        // region variables.
        if dest_ty.flags().contains(has_type_or_const_vars()) {
            continue;
        }
    ...
    from_impls
        ...
        .filter_map(|(src_ty, cc_ty, impl_id, is_from)| {
            ...
            let (thunk_name, cc_thunk_decls, rs_details) = if is_from {
                ...
                .ok()?;
                let static_src_ty =
                    crate::generate_function_thunk::replace_all_regions_with_static(tcx, src_ty);
