...
///
/// ```rust
/// # use crubit_support::bridge::{Encoder, Decoder, transmute_abi, CrubitAbi};
...
pub unsafe trait CrubitAbi {
    ...
    ///
    /// ```rust
    /// # use crubit_support::bridge::{Encoder, Decoder, CrubitAbi};
    ...
    ///
    /// ```rust
    /// # use crubit_support::bridge::{Decoder, Encoder, CrubitAbi};
    /// pub struct Pair<A1, A2>(A1, A2);
    ///
    /// unsafe impl<A1: CrubitAbi, A2: CrubitAbi> CrubitAbi for Pair<A1, A2> {
    ///     unsafe fn decode(self, decoder: &mut Decoder) -> Self::Value {
...
macro_rules! unstable_encode {
    {@ $crubit_abi_expr:expr, $crubit_abi:ty, $expr:expr} => {{
        let mut __crubit_tmp_buffer = [const { ::core::mem::MaybeUninit::<u8>::uninit() }; <$crubit_abi as $crate::bridge::CrubitAbi>::SIZE];
        let __crubit_tmp_value = $expr;
        #[allow(unused_unsafe)]
        unsafe {
            $crate::bridge::internal::encode::<$crubit_abi>(
                $crubit_abi_expr,
...
macro_rules! unstable_return {
    {@ $crubit_abi_expr:expr, $crubit_abi:ty, $cb:expr} => {{
        ...
        #[allow(unused_unsafe)]
        unsafe {
            $crate::bridge::internal::decode::<$crubit_abi>($crubit_abi_expr, __crubit_tmp_buffer.as_ptr() as *const u8)
        }
    }};
}

pub use crate::{unstable_encode, unstable_return};
