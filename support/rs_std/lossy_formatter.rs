...
impl<'scope, 'formatter> LossyFormatter<'scope, 'formatter> {
    ...
    // Public for testing.
    pub(crate) fn write_slice(&mut self, mut data: &[u8]) -> usize {
        let mut written = 0;
    ...
    pub fn write_fill(&mut self, count: usize, data: u8) -> usize {
        ...
        if data.is_ascii() {
            ...
            while written < count && self.writer.write_char(data).is_ok() {
                written += 1;
            }
        } else {
            while written < count && self.write_byte(data) {
                written += 1;
            }
        }
        written
