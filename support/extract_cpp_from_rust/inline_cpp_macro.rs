...
impl TokenCursor {
    ...
    fn next(&mut self) -> Option<proc_macro2::TokenTree> {
        if self.index < self.tokens.len() {
            let res = self.tokens[self.index].clone();
            self.index += 1;
            Some(res)
        } else {
            None
...
fn strip_qualifiers(s: &str) -> (bool, String) {
    let words: Vec<&str> = s.split_whitespace().collect();
    let is_const = words.contains(&"const");
    let clean_words: Vec<&str> =
        words.into_iter().filter(|&w| w != "const" && w != "volatile").collect();
    (is_const, clean_words.join(" "))
}
...
fn map_cpp_type_to_rust_str(cpp_type: &str) -> Result<String, String> {
    ...
    if s.ends_with('*') || s.ends_with('&') {
        ...
        let (is_const, stripped) = strip_qualifiers(base);
    ...
    if s.ends_with('>') && s.contains('<') {
        ...
        let outer = s[..start_idx].trim();
        let inner = s[start_idx + 1..s.len() - 1].trim();
        ...
        for part in parts {
            mapped_parts.push(map_cpp_type_to_rust_str(&part)?);
        }

        let (_is_const, stripped_outer) = strip_qualifiers(outer);
        let rust_outer = map_cpp_type_to_rust_str(&stripped_outer)?;
        return Ok(format!("{}<{}>", rust_outer, mapped_parts.join(", ")));
    }

    let (_is_const, stripped) = strip_qualifiers(&s);
...
fn parse_and_expand(
    ...
    let _body =
        body_group.ok_or_else(|| "Expected braced body after return type".to_string())?.stream();
...
mod tests {
    ...
    fn test_map_cpp_tokens_to_rust() {
        ...
        expect_eq!(check("const int &"), "& i32");
        expect_eq!(check("std::vector<int>"), "std :: vector < i32 >");
        expect_eq!(check("const std::vector<const int*>&"), "& std :: vector <* const i32 >");
        expect_eq!(check("math_utils::Point"), "math_utils :: Point");
        expect_eq!(
