...
fn as_bytes<'a>(r: ::cref::CRef<'a, string>) -> &'a [u8] {
...
fn test_ffi_round_trip_handle_non_utf8() {
    ...
    let s = emplace!(string::ctor_new(&input[..]));
    let mut s2 = emplace!(string::ctor_new(""));
    test_helpers::cpp_std_string_test::CopyRef(&*s, s2.as_mut());
    expect_eq!(&*s, &*s2);
}

#[gtest]
fn test_string_round_trip_ref() {
    let s = emplace!(string::ctor_new("Hello Ref"));
    let r = test_helpers::cpp_std_string_test::ReturnRef(&*s);
    expect_eq!(as_bytes(r), &b"Hello Ref"[..]);
}

#[gtest]
fn test_string_mutate_ref() {
...
#[gtest]
fn test_string_round_trip_ptr() {
    let s = emplace!(string::ctor_new("Hello Ptr"));
    let s_ptr = s.as_ptr();
...
fn test_string_struct() {
    let s = emplace!(string::ctor_new("hello struct"));
...
#[gtest]
fn test_string_needs_drop() {
    expect_eq!(std::mem::needs_drop::<string>(), true);
}

#[gtest]
fn test_string_round_trip_by_value() {
    let s = emplace!(string::ctor_new("hello by value"));
    let r = test_helpers::cpp_std_string_test::RoundTrip(s.as_slice().into());
    expect_eq!(r.as_slice(), &b"hello by value"[..]);
}
