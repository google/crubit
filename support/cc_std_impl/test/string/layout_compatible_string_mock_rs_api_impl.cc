...
__rust_thunk___ZN4test9RoundTripENSt3__u12basic_stringIcNS0_11char_traitsIcEENS0_9allocatorIcEEEE(
    unsigned char* __return_abi_buffer, const unsigned char* s) {
  ::crubit::Decoder __s_decoder(::crubit::BoxedAbi<std::string>::kSize, s);
  ::crubit::Encoder __return_encoder(::crubit::BoxedAbi<std::string>::kSize,
                                     __return_abi_buffer);
  ::crubit::BoxedAbi<std::string>().Encode(
      test::RoundTrip(::crubit::BoxedAbi<std::string>().Decode(__s_decoder)),
      __return_encoder);
}
...
extern "C" void __rust_thunk___ZN4test12StringStructD1Ev(
    struct test::StringStruct* __this) {
