...
fn test_deref() {
    ...
    expect_eq!(*shared_ptr_const::try_as_ref(&shared).unwrap(), 1);
}

#[gtest]
fn test_as_ptr_and_is_null() {
    let shared = test_helpers::shared_ptr_test::create_shared_ptr();
    expect_false!(shared_ptr_const::is_null(&shared));
    expect_eq!(unsafe { *shared_ptr_const::as_ptr(&shared) }, 1);

    let null_shared = test_helpers::shared_ptr_test::create_shared_ptr_void_ptr();
    expect_true!(shared_ptr_const::is_null(&null_shared));
}
...
fn test_release() {
    ...
    expect_false!(shared_ptr_const::is_null(&shared));
    let (ptr, cntrl) = shared_ptr_const::release(&mut shared);
    expect_true!(shared_ptr_const::is_null(&shared));
    expect_eq!(unsafe { *ptr }, 1);
    // Re-wrap and drop to clean up reference count.
    let _rewrapped = unsafe { shared_ptr_const::from_raw_parts(ptr, cntrl) };
}

#[gtest]
fn test_into_raw_parts() {
    let shared = test_helpers::shared_ptr_test::create_shared_ptr();
    let (ptr, cntrl) = shared_ptr_const::into_raw_parts(shared);
    expect_eq!(unsafe { *ptr }, 1);
    // Re-wrap and drop to clean up reference count.
    let _rewrapped = unsafe { shared_ptr_const::from_raw_parts(ptr, cntrl) };
}
