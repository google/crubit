    ...
    Derived: InheritsVirtual<Base>,
{
    fn virtual_upcast(self: *const Derived) -> *const Base {
        // SAFETY: `self` is a valid pointer. `upcast_ptr_to_virtual_base` returns a properly
        // aligned pointer to the `Base` subobject within `self`.
    ...
    Derived: InheritsVirtual<Base>,
{
    fn virtual_upcast(self: *mut Derived) -> *mut Base {
        // SAFETY: `self` is a valid pointer. `upcast_ptr_mut_to_virtual_base` returns a properly
        // aligned pointer to the `Base` subobject within `self`.
...
pub unsafe trait InheritsVirtual<Base> {
    ...
    ///
    /// Otherwise, if `derived` is non-dereferencable, the behavior is undefined.
