...
pub mod internal {
    ...
    impl RapidHasher {
        ...
        pub const fn new() -> Self {
            let seed = Self::rapid_mix(Self::K0 ^ Self::K2, Self::K1);
        ...
        fn consume_8_bytes(&mut self, val: u64) {
            self.seed = Self::rapid_mix(val ^ Self::K0, self.seed ^ Self::K1);
        }
    }
        ...
        fn finish(&self) -> u64 {
            ...
            if self.buf_len > 0 {
                ...
                while i < self.buf_len {
                    last_bytes[i] = self.buffer[i];
                    i += 1;
                }
                let val = u64::from_le_bytes(last_bytes);
                final_seed = Self::rapid_mix(val ^ Self::K0, final_seed ^ Self::K1);
            }
            Self::rapid_mix(final_seed ^ Self::K2 ^ self.total_len, Self::K1)
        }
    ...
    #[inline(always)]
    pub fn hash_u64<T: ::core::hash::Hash + ?Sized>(value: &T) -> u64 {
...
mod tests {
    ...
    fn test_rapid_hasher() {
        let mut hasher = internal::RapidHasher::new();
        hasher.write(&[1, 2, 3]);
        let h1 = hasher.finish();

        let mut hasher2 = internal::RapidHasher::new();
        hasher2.write(&[1, 2, 3]);
        let h2 = hasher2.finish();

        let mut hasher3 = internal::RapidHasher::new();
        hasher3.write(&[1, 2, 4]);
    ...
    #[gtest]
    fn test_hash_u64() {
        let h1 = internal::hash_u64(&42i32);
        let h2 = internal::hash_u64(&42i32);
        let h3 = internal::hash_u64(&43i32);
