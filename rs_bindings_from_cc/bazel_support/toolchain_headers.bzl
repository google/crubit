...
def _bindings_for_toolchain_headers_impl(ctx):
    ...
    bindings_providers = list(bindings_providers)
    for p in list(bindings_providers):
        if hasattr(p, "dep_variant_info"):
            if p.dep_variant_info:
                bindings_providers.append(p.dep_variant_info.crate_info)
                bindings_providers.append(p.dep_variant_info.dep_info)
                bindings_providers.append(p.dep_variant_info.cc_info)
            break
    return [RustToolchainHeadersInfo(headers = std_and_builtin_files)] + bindings_providers
