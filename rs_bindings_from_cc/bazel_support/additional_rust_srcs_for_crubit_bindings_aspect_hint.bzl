...
def _additional_rust_srcs_for_crubit_bindings_impl(ctx):
    ...
    if ctx.attr.warn_on_raw_cc_deps:
        for dep in ctx.attr.deps:
            if CcInfo in dep and CrateInfo not in dep:
                rule_label = str(ctx.label)

                # If the aspect hint was generated by `rust_api_from_cpp`, its name ends with `.hint`.
                # Trim `.hint` so the warning references the user-facing `rust_api_from_cpp` target.
                if rule_label.endswith(".hint"):
                    rule_label = rule_label[:-5]
...
_additional_rust_srcs_for_crubit_bindings_rule = rule(
    attrs = {
        ...
        "warn_on_raw_cc_deps": attr.bool(
            doc = "Whether to emit a build warning when dependencies in `deps` are raw C++ libraries without Rust bindings.",
            mandatory = False,
            default = False,
        ),
    },
...
def additional_rust_srcs_for_crubit_bindings(
    ...
    Args:
        ...
        warn_on_raw_cc_deps: Whether to emit a warning when a direct dependency in `deps` is a raw C++ library without Rust bindings.
    ...
    _additional_rust_srcs_for_crubit_bindings_rule(
        ...
        root_namespaces = root_namespaces,
        aliases = aliases,
        warn_on_raw_cc_deps = warn_on_raw_cc_deps,
        **kwargs
    )
