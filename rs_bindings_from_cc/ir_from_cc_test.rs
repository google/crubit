...
fn test_unescapable_rust_keywords_in_field_name() {
    ...
    let rust_id = field.rust_identifier();
    assert_eq!(rust_id.as_ref().map(|x| x.as_str()), Some("__field_0"));
    let cpp_id = field.cpp_identifier();
    assert_eq!(cpp_id.as_ref().map(|x| x.as_str()), Some("self"));
}
...
fn test_doc_comment() -> Result<()> {
    ...
    let comments: HashMap<_, _> =
        ir.records().map(|r| (r.rs_name_as_str(), r.doc_comment().unwrap())).collect();
...
fn test_multiple_typedefs_to_same_specialization() -> Result<()> {
    ...
    assert_eq!(1, ir.records().filter(|r| r.cc_name_as_str() == "MyStruct<int>").count());
    assert_eq!(1, ir.records().filter(|r| r.cc_name_as_str() == "MyStruct<float>").count());
    let functions = ir
        .functions()
...
fn test_implicit_specialization_items_are_deterministically_ordered() -> Result<()> {
    ...
    let class_template_specialization_names = ir
        ...
        .filter_map(|item| match item {
            ir::Item::Record(r) if r.rs_name_as_str().contains("__CcTemplateInst") => {
                Some(r.rs_name_as_str())
            }
            _ => None,
        })
        .collect_vec();
...
fn test_templates_inheritance() -> Result<()> {
    ...
    assert_eq!(
        1,
        ir.records()
            .filter(|r| r.cc_name_as_str().contains("ClassTemplateDerivedFromClassTemplate"))
            .count()
    );
    ...
    assert_eq!(0, ir.records().filter(|r| r.cc_name_as_str().contains("BaseTemplate")).count());
    Ok(())
}
...
fn test_dont_import_injected_class_name() {
    ...
    let names = ir.records().map(|r| r.rs_name_as_str()).filter(|n| n.contains("SomeStruct"));
...
fn test_struct_forward_declaration() {
    ...
    assert!(!ir.records().any(|r| r.rs_name_as_str() == "Struct"));
}
...
fn assert_member_function_with_predicate_has_instance_method_metadata<F: FnMut(&Func) -> bool>(
    ...
    let record =
        ir.records().find(|r| r.rs_name_as_str() == record_name).expect("record not found");
...
fn test_operator_names() {
    ...
    let operator_names: HashSet<&str> = ir
        ...
        .filter_map(|f| {
            ...
            if r.rs_name_as_str() != "SomeStruct" {
                return None;
            }
...
fn test_enclosing_item_ids() {
    ...
    let record = ir.records().find(|r| r.rs_name_as_str() == "S").unwrap();
