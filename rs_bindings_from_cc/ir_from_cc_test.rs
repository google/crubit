...
fn test_struct_with_owned_ptr_type_annotation() -> googletest::Result<()> {
    let ir = ir_from_cc(
        r#"
        struct [[clang::annotate("crubit_owned_ptr_type", "SomeOwnedPtrType")]]
...
fn test_owned_ptr_as_return_type_annotation() -> googletest::Result<()> {
    let ir = ir_from_cc(
        r#"
        struct SomeType;
        SomeType* [[clang::annotate_type("crubit_owned_ptr")]] f() { return nullptr; }
        "#,
    )
    .expect("Failed to generate IR from CC");
...
fn test_owned_ptr_as_param_type_annotation() -> googletest::Result<()> {
    let ir = ir_from_cc(
        ...
        struct SomeType;
        void f(
            SomeType* [[clang::annotate_type("crubit_owned_ptr")]] owned_ptr
          ) {}
        "#,
    )
    .expect("Failed to generate IR from CC");
