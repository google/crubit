...
pub fn cpp_tagless_type_name_for_record(
    ...
    let ident = expect_format_cc_type_name(record.cc_name().as_str());
...
pub fn tagless_cpp_type_name_for_item(
    ...
    match item {
        Item::IncompleteRecord(incomplete_record) => {
            let ident = expect_format_cc_type_name(incomplete_record.cc_name().as_str());
        ...
        Item::Enum(enum_) => {
            let ident = expect_format_cc_type_name(enum_.rs_name().as_str());
            let namespace_qualifier = db.namespace_qualifier(item).format_for_cc()?;
            Ok(quote! { #namespace_qualifier #ident })
        }
        Item::TypeAlias(type_alias) => {
            let ident = expect_format_cc_type_name(type_alias.cc_name().as_str());
            let namespace_qualifier = db.namespace_qualifier(item).format_for_cc()?;
            Ok(quote! { #namespace_qualifier #ident })
        }
