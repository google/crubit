...
#[gtest]
fn test_existing_rust_type_single_ident_matching_target_name_does_not_infer_hint() -> Result<()> {
    let proto = ir_proto_from_assumed_lifetimes_cc(
        r#"
            struct [[clang::annotate("crubit_internal_rust_type", "testing_target")]]
            MyStruct {};
            void Func(MyStruct x);
        "#,
    )?;
    ...
    let func = retrieve_func(db.ir(), "Func");
    let rs_type = db.rs_type_kind(func.params()[0].type_().clone())?;
    let fmt = rs_type.to_token_stream(&db).to_string();
    expect_eq!("crate :: testing_target", fmt);
    Ok(())
}
