...
pub enum UniformReprTemplateType<'a> {
    ...
    StdAtomic {
        in_cc_std: bool,
        // No lifetime here: owned by the atomic
        element_type: RsTypeKind<'a>,
    },
    /// std::shared_ptr<T>
    StdSharedPtr {
    ...
    fn new(
        ...
        match template_specialization_kind {
            ...
            Some(TemplateSpecializationKind::StdAtomic { raw_element_type }) => {
                let element_type = choose_one_type(raw_element_type, template_args)?;
                let element_type = type_arg(&element_type)?;
                Ok(Some(Rc::new(UniformReprTemplateType::StdAtomic { in_cc_std, element_type })))
            }
...
impl<'a> RsTypeKind<'a> {
    ...
    fn new_type_alias(
        ...
        if let RsTypeKind::Record { record, .. } = &underlying_type
            && record.as_ref().owning_target() != type_alias.as_ref().owning_target()
    ...
    fn new_record(
        ...
        let crate_path = Rc::new(CratePath::new(
            ...
            rs_imported_crate_name(record.as_ref().owning_target(), ir),
        ));

        let in_cc_std = db.ir().is_current_target(record.as_ref().owning_target())
            && record.as_ref().owning_target().target_name_escaped() == "cc_std";
        ...
        let label_hint = if let Some(hint_str) = existing_rust_type.label_hint() {
            ...
            if has_remainder {
                existing_rust_type.owning_target().filter(|owning_target| {
                    first.is_some_and(|part| part == owning_target.target_name())
                })
            } else {
                None
            }
    ...
    ///
    /// For example, `absl::StatusOr<T>` and `std::optional<T>` stores `T` in place, so whether
    /// `std::optional<T>` is `Unpin` depends on whether `T` is `Unpin`.
    ///
