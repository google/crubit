            ...
            let mut body = if return_type.is_c_abi_compatible_by_value() {
                ...
                if let Some(result_type) = return_type.as_c9_co() {
                    let consume_result_fn = if result_type.is_void() {
                        quote! { ::co_lib::internal::consume_void }
                    } else {
                        ...
                        // Here, we're creating a spelling for the co_lib::consume_result function,
                        //explicitly providing the generic arguments. There are two arguments:
                        // - the `A` type that implements ::bridge_rust::CrubitAbi
                        // - `A::SIZE`
                        // Theoretically, we shouldn't need to provide A::SIZE since it can
                        // obviously be gotten from A, but because of shortcomings in Rust's support
                        // for const generics being used in const positions:
                        // https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=f52065765bba85ea38c09571d80acd0d
                        // we need to pass in the size separately. Note that the safety of
                        // consume_result depends on the correct size being passed in. This is
                        // not a big deal though because all calls to this function are intended to
                        // be generated by Crubit, so we don't have to worry about folks
                        // accidentally doing the wrong thing.
                        quote! {
                            ::co_lib::internal::consume_result::<#crubit_abi_type_tokens, {
