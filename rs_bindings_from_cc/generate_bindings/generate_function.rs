...
fn api_func_shape_for_conversion_operator<'a>(
    ...
    // src_type_kind is a reference type (`&Src` or `&mut Src`) on the Rust side.
    ...
    let (trait_name, func_name, src_param_type) = if is_rvalue {
        if referent_type_kind.is_unpin() && is_dst_unpin {
            (
                TraitName::From(Rc::new([referent_type_kind.clone()])),
                make_rs_ident("from"),
                referent_type_kind.clone(),
            )
        } else if referent_type_kind.is_unpin() && !is_dst_unpin {
            (
        ...
        if is_dst_unpin {
            (
                TraitName::From(Rc::new([src_type_kind.clone()])),
                make_rs_ident("from"),
                src_type_kind.clone(),
            )
        } else {
    ...
    let associated_return_type = if matches!(trait_name, TraitName::CtorNew(_)) {
        Some(make_rs_ident("CtorType"))
    } else {
        None
    };
...
fn api_func_shape<'a>(
    ...
    match func.rs_name() {
        ...
        UnqualifiedIdentifier::ConversionOperator => {
            api_func_shape_for_conversion_operator(func, maybe_record, param_types, return_type)
        }
    }
}
    ...
    let param_types: Vec<RsTypeKind> = func
        ...
        .filter_map(|(i, param)| {
            ...
            if i == 0 && func.is_instance_method() {
                ...
                let CcTypeVariant::Pointer(ptr) = param_type.variant_mut() else {
                    panic!(
...
pub fn generate_function<'a>(
    ...
    match impl_kind {
        ...
        ImplKind::Trait {
            ...
            let assume_lifetimes = db
                .ir()
                .target_crubit_features(trait_record.as_ref().owning_target())
                .contains(crubit_feature::CrubitFeature::AssumeLifetimes);
            // TODO(b/454627672): is it worth caching this?
            ...
            let self_type = match &impl_for {
                ...
                _ => {
                    ...
                    syn::parse2(quote! { #record_qualifier #record_name }).ok()
                }
            };
            function_id = FunctionId {
                self_type,
