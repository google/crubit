    ...
    let return_stmt = match kind {
        ThunkCallKind::InlineCpp(ref body_tokens) => {
            if func.cc_name().is_constructor() || func.cc_name().is_destructor() {
                ...
                let this_type_kind = db.rs_type_kind(func.params()[0].type_().clone())?;
                let Some(referent) = this_type_kind.referent() else {
                    bail!("Constructor/destructor `__this` param must be a reference or pointer type.");
                };
                let struct_cpp_type = cpp_type_name::format_cpp_type(referent, db)?;
...
pub fn generate_inline_cpp_call<'a>(
    ...
    let body_block = if func.cc_name().is_constructor() || func.cc_name().is_destructor() {
        ...
        let is_out_param_return = matches!(
            return_type_kind.passing_convention(),
            PassingConvention::ComposablyBridged
                | PassingConvention::LayoutCompatible
                | PassingConvention::Ctor
        );
        match (conversion_stmts.is_empty(), is_out_param_return) {
            (true, true) => quote! { { #return_stmt; } },
            (true, false) => quote! { #return_stmt },
            (false, true) => quote! {
                {
                    #conversion_stmts
                    #return_stmt;
