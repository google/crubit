...
fn decl_lifetime_arity_impl(
    ...
    match item {
        ...
        // rc.is_string_view()).
        Item::Record(rc) => match rc.template_specialization().map(|ts| ts.kind().clone()) {
            Some(
        ...
        Item::IncompleteRecord(_) => {
            bail!("Incomplete records unhandled for lifetimes: {:?}", item.cc_name())
        }
        Item::Enum(_ec) => bail!("Enums unhandled for lifetimes: {:?}", item.cc_name()),
        Item::ExistingRustType(_ec) => {
            bail!("Existing Rust types unhandled for lifetimes: {:?}", item.cc_name())
        }
        Item::Func(_) => {
            bail!("Unexpected function found in type position: {:?}", item.cc_name())
        }
        Item::Comment(_) => {
            bail!("Unexpected comment found in type position: {:?}", item.cc_name())
        }
        Item::Namespace(_) => {
            bail!("Unexpected namespace found in type position: {:?}", item.cc_name())
        }
        Item::UseMod(_) => {
            bail!("Unexpected UseMod found in type position: {:?}", item.cc_name())
        }
        Item::GlobalVar(_) => {
            bail!("Unexpected global var found in type position: {:?}", item.cc_name())
        }
        Item::Constant(_) => {
            bail!("Unexpected const found in type position: {:?}", item.cc_name())
        }
        Item::UnsupportedItem(_) => {
            bail!("Unexpected unsupported item found in type position: {:?}", item.cc_name())
        }
    }
}
...
impl BindingContext {
    ...
    pub fn push_fresh_binding(&mut self, hint: Option<&str>) -> Rc<str> {
        let fresh_name = match hint {
            Some(id) => self.fresh_name_for(id),
            None => self.fresh_name_for("lt"),
        };
    ...
    pub fn fresh_name_for(&self, id: &str) -> Rc<str> {
...
impl<'a, 'db> LifetimeDefaults<'a, 'db> {
        ...
        match ty.variant() {
            CcTypeVariant::Decl { id, .. } if self.decl_binds_lifetimes(id)? => {
                ...
                if let Some(type_arg) = self.type_arg_from_decl_id(id) {
                    ...
                    *new_ty.variant_mut() =
                        CcTypeVariant::Decl { id, template_args: Some(Rc::new([type_arg])) };
                };
                let mut state =
    ...
    fn type_arg_from_decl_id(&mut self, id: ItemId) -> Option<CcType> {
        match self.db.find_untyped_decl(id) {
            Item::Record(record) => match record.template_specialization().map(|ts| ts.kind()) {
                Some(ir::TemplateSpecializationKind::StdVector { raw_element_type, .. }) => {
                    Some(raw_element_type.clone())
                }
                Some(ir::TemplateSpecializationKind::StdUniquePtr { raw_element_type, .. }) => {
                    Some(raw_element_type.clone())
                }
                Some(ir::TemplateSpecializationKind::C9Co { raw_element_type, .. }) => {
                    Some(raw_element_type.clone())
                }
                Some(ir::TemplateSpecializationKind::AbslSpan { raw_element_type, .. }) => {
                    Some(raw_element_type.clone())
                }
                _ => None,
            },
    ...
    /// to types that expect a single lifetime (e.g. pointer, Decl of single-lifetime-arg template).
    fn add_lifetime_to_output_type(
        &mut self,
        lifetime_hint: &[Rc<str>],
        new_bindings: &mut Vec<Rc<str>>,
        ty: &CcType,
    ) -> Result<CcType> {
        match ty.variant() {
            CcTypeVariant::Decl { id, .. } => {
                ...
                if self.decl_binds_lifetimes(&id)? {
                    ...
                    if !ty.explicit_lifetimes().is_empty() {
                        *new_ty.explicit_lifetimes_mut() = ty
                            .explicit_lifetimes()
                            .iter()
                            .map(|l| {
                ...
                if let Some(elt) = self.type_arg_from_decl_id(id) {
                    ...
                    *new_ty.variant_mut() =
                        CcTypeVariant::Decl { id, template_args: Some(Rc::new([elt_lowered])) }
                };
                Ok(new_ty)
            }
    ...
    fn inject_lifetimes_into_this(
        ...
        match param.type_mut().variant_mut() {
            CcTypeVariant::Pointer(pty) => {
                ...
                let mut new_pointee = pty.pointee_type().as_ref().clone();
                *new_pointee.explicit_lifetimes_mut() = lifetimes.to_vec();
                *pty.pointee_type_mut() = Rc::new(new_pointee);
                Ok(())
            }
            _ => {
    ...
    fn add_lifetime_to_func(&mut self, func: &Func) -> Result<Func> {
        ...
        let mut params = std::mem::take(new_func.params_mut());
        let mut lifetime_inputs = std::mem::take(new_func.lifetime_inputs_mut());

        for (ix, param) in params.iter_mut().enumerate() {
            let is_constructor = func.cc_name() == &ir::UnqualifiedIdentifier::Constructor;
        ...
        *new_func.params_mut() = params;
        ...
        let return_type_arg = new_func.return_type().clone();
        let new_return_type =
            self.add_lifetime_to_output_type(&lifetime, &mut lifetime_inputs, &return_type_arg)?;
        *new_func.return_type_mut() = new_return_type;
...
fn transform_item(db: &BindingsGenerator, item: &Item) -> Result<Item> {
    ...
    match &mut transformed {
        Item::Record(record) => {
            ...
            *record.children_mut() = transform_children(db, record.children())?;
            transformed = Item::Record(Rc::new(record));
        }
        Item::Namespace(ns) => {
            let mut ns = Rc::as_ref(ns).clone();
            *ns.children_mut() = transform_children(db, ns.children())?;
