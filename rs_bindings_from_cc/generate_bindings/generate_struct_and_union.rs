...
pub fn generate_incomplete_record(
    ...
    let visibility = db
        ...
        .unwrap_or_default()
        .visibility;
    let cc_type = expect_format_cc_type_name(incomplete_record.cc_name());
    ...
    Ok(ApiSnippets {
        generated_items: HashMap::from([(
            ...
            GeneratedItem::ForwardDeclare {
                visibility,
                ident: make_rs_ident(incomplete_record.rs_name()),
...
fn get_field_rs_type_kind_for_layout(
    ...
    match field.unknown_attr() {
        ...
        Ok(Some(unknown_attr)) => {
            ...
            // features.
            for target in
                db.defining_target(record.id()).as_ref().into_iter().chain([owning_target])
            {
    ...
    for target in db.defining_target(record.id()).as_ref().into_iter().chain([owning_target]) {
        let enabled_features = ir.target_crubit_features(target);
...
fn field_definition(
    ...
    let visibility = if field.access() == AccessSpecifier::Public && field_rs_type_kind.is_ok() {
        db.type_visibility(Record::owning_target(record), field_rs_type_kind.clone().unwrap())
            .unwrap_or_default()
    } else {
        Visibility::PubCrate
    };
...
pub fn generate_record(db: &BindingsGenerator, record: Rc<Record>) -> Result<ApiSnippets> {
        ...
        .filter_map(|field| {
            ...
            Some(FieldWithLayout {
                ...
                description: vec![BitfieldComment {
                    ...
                    field_name: field.rust_identifier().map(|i| Rc::from(i.as_str())),
                    bits: size,
                }],
            })
        })
    ...
    // Both the template definition and its instantiation should enable experimental
    // features.
    let mut crubit_features = ir.target_crubit_features(Record::owning_target(&record));
    ...
    let display_impl = if record.detected_formatter() {
        let fmt_fn_name = make_rs_ident(&format!(
            ...
            odr_suffix = Record::owning_target(&record).convert_to_cc_identifier(),
        ));
        let thunk =
    ...
    let owned_ptr_config =
        record.owned_ptr_config().map(|cfg| database::code_snippet::OwnedPtrConfig {
    ...
    let record_tokens = database::code_snippet::Record {
        ...
        deprecated_attr: record.deprecated().map(DeprecatedAttr),
...
fn cc_struct_layout_assertion(db: &BindingsGenerator, record: &Record) -> Result<ThunkImpl> {
    ...
    Ok(ThunkImpl::LayoutAssertion {
        ...
        namespace_qualifier,
        record_ident: Rc::from(record.cc_name()),
        sizeof_impl,
        size: record.size_align().size,
...
fn cc_struct_operator_delete_impl(
    ...
    let thunk_ident = make_rs_ident(&format!(
        ...
        Record::owning_target(record).convert_to_cc_identifier(),
    ));
