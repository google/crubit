...
pub fn generate_incomplete_record(
    ...
    let visibility = db
        ...
        .unwrap_or_default()
        .visibility;
    let cc_type = expect_format_cc_type_name(incomplete_record.cc_name().as_str());
...
fn field_definition(
    ...
    let deprecated_attr = field.deprecated().map(DeprecatedAttr);
    ...
    let fields_with_bounds: Vec<FieldWithLayout> = record
        ...
        .filter_map(|field| {
            ...
            Some(FieldWithLayout {
                ...
                description: vec![BitfieldComment {
                    field_name: field.rust_identifier().map(|i| std::rc::Rc::from(i.as_str())),
                    bits: size,
                }],
            })
        })
...
fn cc_struct_layout_assertion(db: &BindingsGenerator, record: &Record) -> Result<ThunkImpl> {
    ...
    Ok(ThunkImpl::LayoutAssertion {
        ...
        namespace_qualifier,
        record_ident: std::rc::Rc::from(record.cc_name().as_str()),
        sizeof_impl,
        size: record.size_align().size,
