...
class MemberAccessRewriter
 ...
 public:
  ...
  bool TraverseMemberExpr(clang::MemberExpr* member_expr) {
    auto* field_decl =
        clang::dyn_cast<clang::FieldDecl>(member_expr->getMemberDecl());
    if (field_decl != nullptr && !field_decl->isBitField() &&
        (field_decl->getAccess() == clang::AS_private ||
         field_decl->getAccess() == clang::AS_protected)) {
      std::string record_name =
          field_decl->getParent()->getQualifiedNameAsString();
      std::string mangled_record = record_name;
      for (char& c : mangled_record) {
        if (!absl::ascii_isalnum(c) && c != '_') {
          c = '_';
        }
      }
      std::string tag_name = absl::StrCat("Tag_", mangled_record, "_",
                                          field_decl->getNameAsString());
      std::string replacement = absl::StrCat(
          "(*__this.*get_member_ptr(::crubit_private_tags::", tag_name, "{}))");
    ...
    return clang::RecursiveASTVisitor<MemberAccessRewriter>::TraverseMemberExpr(
        member_expr);
  }
  ...
  bool VisitCXXThisExpr(clang::CXXThisExpr* this_expr) {
    if (!this_expr->isImplicit()) {
      rewriter_.ReplaceText(
          clang::CharSourceRange::getTokenRange(this_expr->getSourceRange()),
          "__this");
    }
    return true;
  }
