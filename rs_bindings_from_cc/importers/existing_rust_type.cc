...
GetCrubitInternalRustTypeAttr(ImportContext& ictx, const clang::Decl& decl) {
  ...
  if (args.empty()) {
    return absl::InvalidArgumentError(
        "The `CRUBIT_INTERNAL_RUST_TYPE` attribute requires at least a single "
        "string literal argument (the Rust type), and optionally a second "
        "argument specifying template arguments.");
  }
  CRUBIT_ASSIGN_OR_RETURN(
  ...
  if (args.size() > 2) {
    return absl::InvalidArgumentError(
        "The `CRUBIT_INTERNAL_RUST_TYPE` attribute may have at most two "
        "arguments.");
  }
  ...
  if (spec == nullptr) {
    return absl::InvalidArgumentError(
        "The second argument of `CRUBIT_INTERNAL_RUST_TYPE` must be a call to "
  ...
  if (spec->getTemplateArgs().size() != 1) {
    return absl::InvalidArgumentError(absl::StrCat(
        "The `crubit::crubit_internal_rust_type_args` template must have "
        "exactly one template pack argument, but has ",
        spec->getTemplateArgs().size(), " template arguments instead."));
  }
  ...
  if (spec_template_arg.getKind() != clang::TemplateArgument::Pack) {
    return absl::InvalidArgumentError(absl::StrCat(
        "The `crubit::crubit_internal_rust_type_args` template must have "
        "exactly one template pack argument, but has a ",
        ArgKindToString(spec_template_arg.getKind()), " argument instead."));
  }
  ...
  for (const auto& arg : pack) {
    ...
    if (const auto* const_generic =
      ...
      if (const_generic->getTemplateArgs().size() != 1) {
        return absl::InvalidArgumentError(
            "`crubit::const_generic` must have exactly one template argument.");
      }
...
std::optional<IR::Item> ExistingRustTypeImporter::Import(
  ...
  return ExistingRustType{
      .rs_name = (**std::move(status_or_opt_attr)).format_string,
      .cc_name = std::move(cc_name),
      .unique_name = ictx_.GetUniqueName(*type_decl),
      .template_args = (**std::move(status_or_opt_attr)).format_args,
      .template_arg_names =
