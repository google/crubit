...
fn test_atomic_u64() -> googletest::Result<()> {
    ...
    let loaded = unsafe { crust::load_atomic(&mut val) };
...
fn test_atomic_bool() -> googletest::Result<()> {
    ...
    let old = unsafe { crust::exchange_atomic_bool(&mut val, true) };
...
fn test_atomic_int() -> googletest::Result<()> {
    ...
    let old = unsafe { crust::fetch_add_atomic_int(&mut val, 5) };
...
fn test_atomic_ptr() -> googletest::Result<()> {
    ...
    let old = unsafe { crust::exchange_atomic_ptr(&mut val, &mut y) };
...
fn test_edge_cases_compilation() -> googletest::Result<()> {
    ...
    verify_that!(std::mem::size_of::<crust::EdgeCasesContainer>() > 0, eq(true))
}
